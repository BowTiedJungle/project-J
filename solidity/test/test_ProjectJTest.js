...
// console.log(findDuplicates(fullDegenimals))
for (i=0;i<degens.length;i++) {
    fullDegenimals.push(degens[i])
}
...
function generateMerkleTree() {
    const merkleTree = new MerkleTree(
        fullDegenimals.map(hashAddress),
        keccak256,
        { 
            sortPairs: true,
            sortLeaves: true
...
describe("ProjectJ Upgrade Test", function () {
    ...
    beforeEach(async function () {
        ...
        ProjectJTest = await ethers.getContractFactory("ProjectJUpgrade");
